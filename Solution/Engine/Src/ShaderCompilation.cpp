...
namespace Rath
{
	...
	template<typename T> void CompileShader(ID3D11Device* device, CompiledShader* shader)
	{
		...
		for (size_t fileIdx = 0; fileIdx < filePaths.size(); ++fileIdx)
		{
			...
			for (size_t shaderFileIdx = 0; shaderFileIdx < ShaderFiles.size(); ++shaderFileIdx)
			{
			...
			for (size_t shaderIdx = 0; shaderIdx < shaderFile->Shaders.size(); ++shaderIdx)
			{
	...
	void UpdateShaders(ID3D11Device* device)
	{
		...
		static size_t currFile = 0;
		currFile = (currFile + 1) % size_t(ShaderFiles.size());
	...
	void CompileOptions::Add(const std::string& name, uint32 value)
	{
		...
		for (size_t i = 0; i < stringVal.length(); ++i)
