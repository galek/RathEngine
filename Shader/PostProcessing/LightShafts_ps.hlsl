...
Texture2D<float4> g_BackBufferTexture : register(t0);
...
float4 main(VS_OUTPUT Input) : SV_TARGET
{
	...
	float3 color = (float3)g_LightTexture.Sample(g_samPoint, texCoord);
	...
	for (uint i = 0; i < NUM_SAMPLES; i++)
	{
		...
		float3 sample = (float3)g_LightTexture.Sample(g_samPoint, texCoord);
	...
	float3 backBuffer = (float3)g_BackBufferTexture.Sample(g_samPoint, Input.texCoord);
