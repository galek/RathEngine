...
VS_OUTPUT main(VS_INPUT input)
{
	...
	float4 vPositionWS = mul(float4(input.Position, 1.f), input.World);
	float3 vTangentWS = mul(input.Tangent, (float3x3)input.World);
	float3 vBinormalWS = mul(input.Binormal, (float3x3)input.World);
	float3 vNormalWS = normalize(cross(vTangentWS, vBinormalWS));
	...
	output.LTexture = frac(vPositionWS.xyz / float3(32.0f, 256.0f, 32.0f)) * float3(32.0f, 256.0f, 32.0f) + float3(1.0f, 0.0f, 1.0f) + vNormalWS * 0.5f;
